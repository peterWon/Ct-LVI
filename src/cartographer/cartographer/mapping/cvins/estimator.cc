...
Estimator::Estimator(
    ...
    const std::shared_ptr<FeatureManager> fm): ps_(ps), f_manager(fm){
...
void Estimator::setParameter(){
  ...
  // LOG(INFO)<<ps_->FOCAL_LENGTH;
  // LOG(INFO)<<tic[0];
  // LOG(INFO)<<ric[0];
  ProjectionFactor::sqrt_info = ps_->FOCAL_LENGTH / 1.5 * Matrix2d::Identity();
  ProjectionTdFactor::sqrt_info = ps_->FOCAL_LENGTH / 1.5 * Matrix2d::Identity();
  td = ps_->TD;
}
...
void Estimator::processImage(const map<int, vector<pair<
    int, Eigen::Matrix<double, 8, 1>>>> &image, const double header){
  //TODO: the optimization is time-consuming, lock the feature manager
  // here may block the LVI optimization for long time.
  std::scoped_lock lock(f_manager->mtx_);
  if (f_manager->addFeatureCheckParallax(frame_count, image, td, header))
  ...
  if (solver_flag == INITIAL){
    if (frame_count == WINDOW_SIZE){
      bool result = false;
      if( ps_->ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1){
      ...
      if(result){ 
        ...
        LOG(INFO)<<"VINS initialization finished!";
    ...
    if (failureDetection()){
        LOG(WARNING)<<"VINS failure detected!";
        failure_occur = 1;
        clearState();
        setParameter();
        LOG(WARNING)<<"VINS reboot!";
        return;
    }
    
    // align to cartographer
    bool aligned_to_local = false;
    for(int i = 0; i < WINDOW_SIZE + 1; ++i){
      double ts = Headers[i];
      auto it = states_in_local_.find(ts);
      if(it != states_in_local_.end()){
        // align with local frame.
        Eigen::Matrix4d T_i_l = Eigen::Matrix4d::Identity();
        Eigen::Matrix4d T_i_v = Eigen::Matrix4d::Identity();
        T_i_l.block(0, 0, 3, 3) = it->second.Q.toRotationMatrix();
        T_i_l.block(0, 3, 3, 1) = it->second.P;
        T_i_v.block(0, 0, 3, 3) = Rs[i];
        T_i_v.block(0, 3, 3, 1) = Ps[i];
        T_v2l_ = T_i_l * (T_i_v.inverse());
        
        aligned_to_local = true;
        break;
      }
    }
    ...
    if(aligned_to_local){
      for(int i = 0; i < WINDOW_SIZE + 1; ++i){
        Ps[i] = T_v2l_.topLeftCorner(3,3) * Ps[i] + T_v2l_.block(0,3,3,1);
        Vs[i] = T_v2l_.topLeftCorner(3,3) * Vs[i];
        Rs[i] = T_v2l_.topLeftCorner(3,3) * Rs[i];
      }
      T_v2l_.setIdentity();
    }
...
bool Estimator::initialStructure(){
  TicToc t_sfm;
  // using LIO results to initialize directly.
  /* {
    bool aligned_to_local = true;
    ...
    for(int i = 0; i < WINDOW_SIZE+1; ++i){
      ...
      if(it != states_in_local_.end()){
        // align with local frame.
        Ps[i] = it->second.P;
        Rs[i] = it->second.Q.toRotationMatrix();
        Vs[i] = it->second.V;
        Bas[i] = it->second.Ba;
        Bgs[i] = it->second.Bg;
        ...
        ric[0] = ps_->RIC[0];
        // f_manager->setRic(ric);
        f_manager->triangulate(Rs, Ps, &(TIC_TMP[0]), &(ps_->RIC[0]));
      }else{
        aligned_to_local = false;
        break;
      }
    }
    if(aligned_to_local){
      // check this, may be positive defined in vins
      g_ << 0, 0, ps_->G.norm(); 
      T_v2l_ = Eigen::Matrix4d::Identity();
      LOG(INFO)<<"Aligned VINS to cartographer.";
      return true;
    }
  } */
...
bool Estimator::visualInitialAlign(){
  ...
  for(int i = 0; i < WINDOW_SIZE + 1; ++i){
    ...
    if(it != states_in_local_.end()){
      ...
      LOG(INFO)<<"Aligned VINS to cartographer at "<<i<<"-th frame.";
      aligned_to_local = true;
      break;
    }
  }
  ...
  if(aligned_to_local){
    for(int i = 0; i < WINDOW_SIZE + 1; ++i){
      ...
      Vs[i] = T_v2l_.topLeftCorner(3,3) * Vs[i];
      Rs[i] = T_v2l_.topLeftCorner(3,3) * Rs[i];
      // Do we need to repropagate again here?
      // double t = Headers[i];
      // auto it = states_in_local_.find(t);
      // if(it != states_in_local_.end()){
      //   Bas[i] = it->second.Ba;
      //   Bgs[i] = it->second.Bg;
    ...
    // reset T_v2l_ to Identity matrix.
    T_v2l_.setIdentity();
  }
  return true;
}
...
void Estimator::solveOdometry(){
  ...
  if (solver_flag == NON_LINEAR){
    TicToc t_tri;
    f_manager->triangulate(Rs, Ps, tic, ric);
    optimization();
    // debug
    /* for (auto &it_per_id : f_manager->feature){
      it_per_id.used_num = it_per_id.feature_per_frame.size();
      ...
      Eigen::Vector3d pts_camera_i = pts_i * it_per_id.estimated_depth;
      ...
      for (auto &it_per_frame : it_per_id.feature_per_frame){
        ...
        Vector3d pts_j = it_per_frame.point;
        Eigen::Vector3d pts_imu_i = ric[0] * pts_camera_i + tic[0];
        Eigen::Vector3d pts_w = Rs[imu_i] * pts_imu_i + Ps[imu_i];
        Eigen::Vector3d pts_imu_j = Rs[imu_j].transpose() * (pts_w - Ps[imu_j]);
        Eigen::Vector3d pts_camera_j = ric[0].transpose() * (pts_imu_j - tic[0]);
        double dep_j = pts_camera_j.z();
        Eigen::Vector2d residual = (pts_camera_j / dep_j).head<2>() - pts_j.head<2>();
        LOG(INFO)<<residual.x()<<","<<residual.y();
      }
    } */
  }
}
...
void Estimator::double2vector(){
  ...
  if (abs(abs(origin_R0.y()) - 90) < 1.0 
    ...
    rot_diff = Rs[0] * Quaterniond(para_Pose[0][6], para_Pose[0][3], 
      para_Pose[0][4], para_Pose[0][5]).toRotationMatrix().transpose();
  }
  ...
  for (int i = 0; i < NUM_OF_CAM; i++){
    ...
    ric[i] = Quaterniond(para_Ex_Pose[i][6], para_Ex_Pose[i][3],
        para_Ex_Pose[i][4], para_Ex_Pose[i][5]).toRotationMatrix();
  }
  ...
  if(relocalization_info){ 
    ...
    drift_correct_yaw = Utility::R2ypr(prev_relo_r).x() 
    ...
    relo_relative_yaw = Utility::normalizeAngle(Utility::R2ypr(
        Rs[relo_frame_local_index]).x() - Utility::R2ypr(relo_r).x());
    relocalization_info = 0;    
  }
}
...
bool Estimator::failureDetection(){
  ...
  if (Bas[WINDOW_SIZE].norm() > ps_->BIAS_ACC_THRESHOLD){
    LOG(WARNING)<<" big IMU acc bias estimation "<<Bas[WINDOW_SIZE].norm();
    return true;
  }
  if (Bgs[WINDOW_SIZE].norm() > ps_->BIAS_GYR_THRESHOLD){
    LOG(WARNING)<<" big IMU gyr bias estimation "<<Bgs[WINDOW_SIZE].norm();
    return true;
  }
  /*
  if (tic(0) > 1)
  ...
  */
  ...
  if ((tmp_P - last_P).norm() > 5){
    LOG(WARNING)<<" big translation "<<(tmp_P - last_P).norm();
    return true;
  }
...
void Estimator::optimization(){
  ...
  //边缘化过，对Hessian矩阵产生了约束,需要将上一次产生的约束转移到当前优化问题中联合优化
  ...
  if(ps_->ENABLE_STATE_FACTOR){
    std::vector<double> weights = {ps_->WEIGHT_POS, ps_->WEIGHT_ROT, 
        ps_->WEIGHT_VEL, ps_->WEIGHT_BA, ps_->WEIGHT_BG};
    for(int i = 0; i < WINDOW_SIZE + 1; i++){
      auto it = states_in_local_.find(Headers[i]);
      if(it != states_in_local_.end()){
        problem.AddResidualBlock(
          NodeStateCostFunction::CreateAutoDiffCostFunction(it->second.P, 
            it->second.Q, it->second.V, it->second.Ba, it->second.Bg, weights),
            NULL, para_Pose[i], para_SpeedBias[i]);
      }
    }
  }
    ...
    for (auto &it_per_frame : it_per_id.feature_per_frame){
      ...
      if (ps_->ESTIMATE_TD){
        ProjectionTdFactor *f_td = new ProjectionTdFactor(
          ...
          it_per_frame.uv.y(), ps_->ROW, ps_->COL, ps_->TR);
        ...
        if(ps_->FIX_DEPTH_ASSOCIATED && it_per_id.is_depth_associated)
  ...
  if(relocalization_info){
    ...
    for (auto &it_per_id : f_manager->feature){
      it_per_id.used_num = it_per_id.feature_per_frame.size();
      if (!(it_per_id.used_num >= 2 && it_per_id.start_frame 
          < WINDOW_SIZE - 2)){
        continue;
      }
      ...
      if(start <= relo_frame_local_index){   
        ...
        if((int)match_points[retrive_feature_index].z() 
          ...
          problem.AddResidualBlock(f, loss_function, para_Pose[start], 
              relo_Pose, para_Ex_Pose[0], para_Feature[feature_index]);
          retrive_feature_index++;
        }     
      }
    }
  ...
  options.max_num_iterations = ps_->NUM_ITERATIONS;
  //options.use_explicit_schur_complement = true;
  //options.minimizer_progress_to_stdout = true;
  //options.use_nonmonotonic_steps = true;
  if (marginalization_flag == MARGIN_OLD)
    options.max_solver_time_in_seconds = ps_->SOLVER_TIME * 4.0 / 5.0;
  else
    options.max_solver_time_in_seconds = ps_->SOLVER_TIME;
  TicToc t_solver;
  ...
  //咋感觉相当于所有被边缘化的历史帧的信息都通过不断边缘化与后续的帧联系了起来？
  //不是的，当所marg掉的变量之间没有直接联系时，会随着marg的过程，Hessian矩阵稀疏性得到保持。(见崔的解析文档)
  if (marginalization_flag == MARGIN_OLD){
    MarginalizationInfo *marginalization_info = new MarginalizationInfo();
    vector2double();
    //上一次边缘化后产生的等价约束在当前边缘化中会被边缘化掉吗？具体过程会是什么样的呢？哪些矩阵块会受到影响？
    if (last_marginalization_info){
      vector<int> drop_set;
      ...
      ResidualBlockInfo *residual_block_info = 
        new ResidualBlockInfo(marginalization_factor, NULL, 
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame){
          ...
          if (ps_->ESTIMATE_TD){
            ProjectionTdFactor *f_td = new ProjectionTdFactor(
              ...
              it_per_frame.uv.y(), ps_->ROW, ps_->COL, ps_->TR);
            ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
              f_td, loss_function, vector<double *>{para_Pose[imu_i], 
              para_Pose[imu_j], para_Ex_Pose[0], para_Feature[feature_index], 
              para_Td[0]}, vector<int>{0, 3});
            marginalization_info->addResidualBlockInfo(residual_block_info);
          }else{
            ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
            ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(
              f, loss_function, vector<double *>{para_Pose[imu_i], 
              para_Pose[imu_j], para_Ex_Pose[0], para_Feature[feature_index]},
              vector<int>{0, 3});
            marginalization_info->addResidualBlockInfo(residual_block_info);
          }
      ...
      for (int i = 0; i <= WINDOW_SIZE; i++){
        if (i == WINDOW_SIZE - 1)
          ...
          addr_shift[reinterpret_cast<long>(
            para_SpeedBias[i])] = para_SpeedBias[i - 1];
        }else{
          addr_shift[reinterpret_cast<long>(para_Pose[i])] = para_Pose[i];
          addr_shift[reinterpret_cast<long>(
            para_SpeedBias[i])] = para_SpeedBias[i];
        }
      }
      for (int i = 0; i < NUM_OF_CAM; i++)
        addr_shift[reinterpret_cast<long>(para_Ex_Pose[i])] = para_Ex_Pose[i];
...
void Estimator::setReloFrame(double _frame_stamp, int _frame_index, 
    vector<Vector3d> &_match_points, Vector3d _relo_t, Matrix3d _relo_r){
