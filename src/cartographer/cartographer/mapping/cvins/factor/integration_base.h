...
class IntegrationBase
{
  public:
    ...
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                    ...
                    const double ACC_N, const double GYR_N,
    ...
    {
        G_ = G;
        //误差阵。加速度噪声（ACC_N）、陀螺仪噪声（GYR_N）、加速度飘移噪声（ACC_W）、陀螺仪飘移噪声（GYR_W）
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //这里的加速度是去除重力了的吗？若记积分的初始位姿为I或（1,0,0,0），那么delta_q就是当前的姿态
        //un_acc_0和un_acc_1为世界坐标系下的加速度，加速度的原始测量值是局部坐标系下的
        ...
        // 位置和速度积分，按照测量值和bias值，根据运动方程式直接推算
        ...
        // 雅克比的计算参考https://github.com/StevenCui/VIO-Doc
        if(update_jacobian)
        {
    ...
    //传入的Pi,Qi,Vi,Bai,Bgi,Pj...Bgj为当前估计值，返回IMU的测量残差
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (0.5 * G_ * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt) - corrected_delta_p;
        residuals.block<3, 1>(O_R, 0) = 2 * (corrected_delta_q.inverse() * (Qi.inverse() * Qj)).vec();
        residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (G_ * sum_dt + Vj - Vi) - corrected_delta_v;
