...
///                ROS_BREAK();
            ...
            if (jacobians[0])//对i帧的位姿的雅克比（p_i,q_i,），15*7，误差方程15维，变量7维
            {
                Eigen::Map<Eigen::Matrix<double, 15, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);
                jacobian_pose_i.setZero();

                jacobian_pose_i.block<3, 3>(O_P, O_P) = -Qi.inverse().toRotationMatrix();
                jacobian_pose_i.block<3, 3>(O_P, O_R) = Utility::skewSymmetric(Qi.inverse() * (0.5 * pre_integration->G_ * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));
            ...
            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
                ...
                jacobian_pose_i.block<3, 3>(O_V, O_R) = Utility::skewSymmetric(Qi.inverse() * (pre_integration->G_ * sum_dt + Vj - Vi));
            ...
            if (jacobians[1])//对V,bias_a, bias_g的雅克比，各占3维，15*9型
            {
