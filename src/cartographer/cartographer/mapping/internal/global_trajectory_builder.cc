...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
  ...
  void AddSensorData(
    ...
    // LOG(WARNING) <<  "LIO Avg Time "<<time_cost_avg_<<"s.";
  ...
  void AddSensorData(const std::string& sensor_id,
                     const sensor::ImageData& image_data) override {
    CHECK(local_trajectory_builder_);
    std::shared_ptr<Submap> matching_submap = nullptr;
    sensor::ImageFeatureData img_feature_data;
    local_trajectory_builder_->AddImageData(
        image_data, matching_submap, img_feature_data);
    if(matching_submap){
      pose_graph_->AddImageData(
        trajectory_id_, img_feature_data, matching_submap);
    }
  }
