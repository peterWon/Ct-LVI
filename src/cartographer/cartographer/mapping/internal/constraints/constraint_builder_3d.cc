...
void ConstraintBuilder3D::DispatchScanContextMatching(
  ...
  auto sc_ctx_constraints_task = common::make_unique<common::Task>();
  sc_ctx_constraints_task->SetWorkItem([=]() EXCLUDES(mutex_) { 
    ComputeScanContextConstraints(node_id, node_submap_id, constant_data);
  });
  auto sc_ctx_constraints_task_handle =
      thread_pool_->Schedule(std::move(sc_ctx_constraints_task));
  tasks_tracker_.push_back(sc_ctx_constraints_task_handle);
  when_done_task_->AddDependency(sc_ctx_constraints_task_handle);
}
...
void ConstraintBuilder3D::DispatchScanMatcherConstruction(
  ...
  if (submap_scan_matchers_.count(visual_loop.ref_submap) == 0) {
    return;
  }
  
  auto visual_constraints_task = common::make_unique<common::Task>();
  visual_constraints_task->SetWorkItem([=]() EXCLUDES(mutex_) { 
    ComputeConstraintsForVisualLoop(visual_loop);
  });
  auto visual_constraints_task_handle =
      thread_pool_->Schedule(std::move(visual_constraints_task));
  tasks_tracker_.push_back(visual_constraints_task_handle);
  when_done_task_->AddDependency(visual_constraints_task_handle);
}
...
void ConstraintBuilder3D::ComputeConstraintsBetweenSubmaps(
  ...
  if(scan_matcher_from.matched_submaps_5dof.empty()) return;
  
  for(const auto& submap_id_to: scan_matcher_from.matched_submaps_5dof){
    ...
    for(const auto& node: scan_matcher_from.nodes_in_submap){
      ...
      // TODO(wz): once one node of this submap is successfully matched,
      // the left ones thus have full 6-DoF initial guesses. 
      // So, we can further speedup the loop constraint construction.
...
void ConstraintBuilder3D::ComputeConstraintsForVisualLoop(
      const SE3SubmapConstraint& visual_loop) EXCLUDES(mutex_){
  
  SubmapId submap_id_to = visual_loop.ref_submap;
  if(submap_scan_matchers_.find(submap_id_to) == submap_scan_matchers_.end()){
    LOG(WARNING)<<"This should not happen!";
    return;
  };
  
  int j = 0;
  for(const auto& node: visual_loop.nodes_in_submap){
    if(j++ % options_.every_nodes_to_find_constraint() != 0) continue;
    // constraint between the node and the submap has been added before.
    const auto& it = computed_constraints_.find(submap_id_to);
    if(it != computed_constraints_.end() 
      && it->second.find(node.first) != it->second.end()){
      continue;
    }
    ...
    constraint_task->SetWorkItem([=]() 
      EXCLUDES(mutex_) {
      ComputeConstraint(submap_id_to, node.first, visual_loop, constraint);
    });
...
void ConstraintBuilder3D::ComputeConstraint(
    ...
    const SE3SubmapConstraint& visaul_loop,
  ...
  std::unique_ptr<scan_matching::FastCorrelativeScanMatcher3D::Result>
      match_result;
  const auto& submap_to_matcher = submap_scan_matchers_.at(submap_id);
  
  transform::Rigid3d node_pose_in_submap_obs;
  transform::Rigid3d node_pose_in_submap_ref_se3;
  
  std::shared_ptr<const TrajectoryNode::Data> constant_data = nullptr;
  const auto& nodes = visaul_loop.nodes_in_submap;
  for(int i = 0; i < nodes.size(); ++i){
    if(nodes.at(i).first == node_id){
      constant_data = nodes.at(i).second.constant_data;
      node_pose_in_submap_obs = nodes.at(i).second.global_pose;
      break;
    }
  } 
  CHECK(constant_data != nullptr) << "Invalid constant_data!";
  
  // pose of the observation submap to the reference submap.
  transform::Rigid3d obs_to_ref = visaul_loop.T_ref_in_refmap 
      * visaul_loop.T_obs_ref * (visaul_loop.T_obs_in_obsmap.inverse());
  node_pose_in_submap_ref_se3 = obs_to_ref * node_pose_in_submap_obs;
  ...
  // 3. Refine.
  {
    ...
    if(visaul_loop.is_translation_usable){
      match_result = submap_to_matcher.fast_correlative_scan_matcher
        ->MatchWith6DoFInitial(node_pose_in_submap_ref_se3, 
              *constant_data, options_.min_score());
    }else{
      match_result = submap_to_matcher.fast_correlative_scan_matcher
      ->MatchWith3DoFInitial(node_pose_in_submap_ref_se3.rotation(), 
              *constant_data, options_.min_score());
    }    
    
    if(match_result){
      if(visaul_loop.is_translation_usable){
        LOG(WARNING)<<"Add a 6-DoF visual loop constraint.";
      }else{
        LOG(WARNING)<<"Add a 3-DoF visual loop constraint.";
      }
    }
...
void ConstraintBuilder3D::ComputeConstraint(
    const SubmapId& submap_id, const NodeId& node_id, 
    const SubmapId& node_submap_id /*submap_id that the node belongs to*/,
    std::unique_ptr<Constraint>* constraint) {
  cartographer::common::TicToc tic_toc;
  tic_toc.Tic();
  // The 'constraint_transform' (submap i <- node j) is computed from:
  ...
  SubmapScanMatcher& submap_from_matcher 
      = submap_scan_matchers_.at(node_submap_id);

  auto iter_5dof = submap_from_matcher.matched_submaps_5dof.find(submap_id);
  auto iter_6dof = submap_from_matcher.matched_submaps_6dof.find(submap_id);
  if(iter_5dof == submap_from_matcher.matched_submaps_5dof.end()) {
    LOG(WARNING) << 
      "No matching between submaps, should not get into this function.";
    return;
  }
  
  transform::Rigid3d node_pose_in_submap_obs;
  transform::Rigid3d node_pose_in_submap_ref_5dof;
  transform::Rigid3d node_pose_in_submap_ref_6dof;
  ...
  if(iter_6dof == submap_from_matcher.matched_submaps_6dof.end()){
    transform::Rigid3d submap_to_submap_3DOF = iter_5dof->second;
    
    // P in submap to match
    // T_G1_S1 * T_G2_G1 * T_S2_G2 * T_N_S2 * P
    transform::Rigid3d gravity_aligned_obs = transform::Rigid3d::Rotation(
        submap_from_matcher.global_submap_pose.rotation());
    transform::Rigid3d gravity_aligned_ref = transform::Rigid3d::Rotation(
        submap_to_matcher.global_submap_pose.rotation());
    
    // Remove yaw in map frame
    double yaw_obs = transform::GetYaw(gravity_aligned_obs);
    auto inv_yaw_obs_rot = transform::Embed3D(
        transform::Rigid2d::Rotation(-yaw_obs));
    gravity_aligned_obs = inv_yaw_obs_rot * gravity_aligned_obs;

    double yaw_ref = transform::GetYaw(gravity_aligned_ref);
    auto inv_yaw_ref = transform::Embed3D(
        transform::Rigid2d::Rotation(-yaw_ref));
    gravity_aligned_ref = inv_yaw_ref * gravity_aligned_ref;

    const auto& T_G1_S1 = gravity_aligned_ref.inverse();
    const auto& T_S2_G2 = gravity_aligned_obs;

    // restore 5-DoF estimate.
    node_pose_in_submap_ref_5dof = T_G1_S1 * submap_to_submap_3DOF 
        * T_S2_G2 * node_pose_in_submap_obs;
  }else{
    node_pose_in_submap_ref_6dof = iter_6dof->second * node_pose_in_submap_obs;
  }
  ...
  // 3. Refine.
  {
    ...
    if(iter_6dof == submap_from_matcher.matched_submaps_6dof.end()){
      match_result = submap_to_matcher.fast_correlative_scan_matcher
        ->MatchWith5DoFInitial(
          node_pose_in_submap_ref_5dof.rotation(),
          node_pose_in_submap_ref_5dof.translation()[0],
          node_pose_in_submap_ref_5dof.translation()[1],
          *constant_data, options_.min_score());
    }else{
      match_result = submap_to_matcher.fast_correlative_scan_matcher
        ->MatchWith6DoFInitial(node_pose_in_submap_ref_6dof, 
              *constant_data, options_.min_score());
    }
    
    if(match_result){
      // if(iter_6dof == submap_from_matcher.matched_submaps_6dof.end()){
      //   LOG(WARNING)<<"Add a 5-DoF scan loop constraint.";
      // }else{
      //   LOG(WARNING)<<"Add a 6-DoF scan loop constraint.";
      // }
    }
  ...
  computed_constraints_[submap_id].insert(node_id);
  transform::Rigid3d obsmap_in_refmap = 
      constraint_transform * node_pose_in_submap_obs.inverse();
  submap_from_matcher.matched_submaps_6dof[submap_id] = obsmap_in_refmap;
...
void ConstraintBuilder3D::ExtractFeaturesForSubmap(
  ...
  cartographer::common::TicToc tic_toc;
  tic_toc.Tic();
  // If there are visual loop closures between a submap and another one, 
  // we prefer to use visual loop constraints rather than 2D submap constraints.
  ...
  if(submap_scan_matchers_.at(submap_id).prj_grid.empty()){
    ...
    // cv::imwrite("/media/wz/Samsung_T5/Data/Tongji-LVI/tj-river-right/submaps/"+std::to_string(submap_id.submap_index)+".jpg", grid);
    cv::threshold(
        grid, grid, options_.cv_binary_threshold(), 255, CV_THRESH_BINARY_INV);
    ...
    for(const auto& entry: submap_scan_matchers_){
      ...
      // Establish 2D submap-to-submap constraints.
      std::vector< std::vector<cv::DMatch> > knn_matches={};

      // TODO(WZ) How about the submap to match is not finished yet in other thread.
      if(submap_scan_matchers_.at(id).descriptors.empty()) continue;
      ...
      if(good_matches.size() >= options_.minimum_good_match_num()){ 
        ...
        if(!transform.empty()){
          ...
          if(std::abs(scale - 1) > options_.scale_estimated_tolerance()) {
            ...
            // insert 2D submap-to-submap constraint, 5-DoF
            submap_scan_matchers_.at(submap_id).matched_submaps_5dof[id]
              = transform::Embed3D(rt2);
            LOG(WARNING)<<"Find scan loop between "<<submap_id.submap_index
                        <<" and "<<id.submap_index;
          }
        }
      }
