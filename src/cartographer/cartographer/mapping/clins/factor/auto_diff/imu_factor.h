...
//TODO: move this parameter to parameter server
// const double GRAVITY_NORM = -9.805;
...
Eigen::Matrix<T, 3, 1> refined_gravity(
  ...
  return Eigen::Matrix<T, 3, 1>(-sp * cr * GRAVITY_NORM,
                                sr * GRAVITY_NORM,
                                -cr * cp * GRAVITY_NORM);
}
...
class GyroAcceWithConstantBiasFactor : public CeresSplineHelper<_N> {
 public:
  ...
  bool operator()(T const* const* sKnots, T* sResiduals) const {
    ...
    Vec3T gravity;
    gravity << T(0.), T(0.), T(gravity_norm_);
#else
...
class GyroAcceBiasFactor : public CeresSplineHelper<_N> {
 public:
  ...
  GyroAcceBiasFactor(const IMUData& imu_data, const IMUBias& imu_bias,
                     ...
                     const double gravity_norm,
                     double gyro_weight,
                     double acce_weight, double ba_weight, double bg_weight)
      : imu_data_(imu_data),
        imu_bias_(imu_bias),
        spline_meta_(spline_meta),
        gravity_norm_(gravity_norm),
  ...
  bool operator()(T const* const* sKnots, T* sResiduals) const {
    ...
    residuals.template block<3, 1>(6, 0) =
        T(bg_weight_) * (gyro_bias - imu_bias_.gyro_bias.template cast<T>());
    residuals.template block<3, 1>(9, 0) =
        T(ba_weight_) * (acce_bias - imu_bias_.accel_bias.template cast<T>());
