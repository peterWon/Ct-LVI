...
class Trajectory : public basalt::Se3Spline<_N, double> {
 public:
  ...
  SE3d GetLidarPose(const double timestamp) const {
    double t = timestamp + calib_param_->time_offset_lidar;
    if (t < this->minTime()) {
      t = this->minTime();
  ...
  bool GetCameraPose(const double timestamp, SE3d &camera_pose) {
    double t = timestamp + calib_param_->time_offset_camera;
    if (t < this->minTime() || t >= this->maxTime()) return false;

    SE3d pose_I_to_G = this->pose(t);
    SE3d pose_C_to_G = pose_I_to_G * calib_param_->se3_CtoI;
    camera_pose = pose_C_to_G;
    return true;
  }
