...
class ImuStateEstimator {
 public:
  ...
  ImuStateEstimator(const cvins::ParameterServer* ps);
  void Reset();
  ...
  double GetLatestTimestamp() const {return latest_imu_time_;}
  IMUState GetLatestState() const {return latest_state_;}
  ...
  MotionState GetCurrentMotionState() const {return motion_state_;}
