...
ImuStateEstimator::ImuStateEstimator(const cvins::ParameterServer* ps)
    : latest_imu_time_(0),
      motion_state_(MotionState::start_motionless),
      accel_excite_threshold_(0.2),
      gyro_excite_threshold_(0.2),
      sample_num_(5) {
  accel_excite_threshold_ = ps->accel_excite_threshold;
  gyro_excite_threshold_ = ps->gyro_excite_threshold;
  sample_num_ = ps->sample_num;
}

void ImuStateEstimator::Reset(){
  latest_imu_time_ = 0;
  imu_data_.clear();
  motion_state_ = MotionState::start_motionless;
