#ifndef __MSCKF_CORNER_DETECTOR__
#define __MSCKF_CORNER_DETECTOR__
...
#include "cartographer/mapping/msckf/fast/fast.h"
...
//                       OutFeatureVector;
typedef std::vector<Eigen::Vector2f> OutFeatureVector;
...
class CornerTracker
{
  public:
    ...
    void track_features(cv::Mat img_1, cv::Mat img_2, Point2fVector& points1, Point2fVector& points2, IdVector& id1, IdVector& id2, const cv::Mat& mask);
...
class TrackHandler
{
  public:
    ...
    TrackHandler(const std::string intrinsic_config_path);
    ~TrackHandler();
    
    camodocal::CameraPtr get_camera(){return camera_;}
    void add_gyro_reading(Eigen::Vector3f& gyro_reading);
    
    void set_mask(const cv::Mat& mask) {mask.copyTo(mask_);};

    void set_current_image(cv::Mat img, double time);
    void tracked_features(
