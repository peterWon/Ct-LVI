...
#include <cartographer/mapping/clins/camera_models/PinholeCamera.h>
#include <cartographer/mapping/clins/camera_models/EquidistantCamera.h>
#include <cartographer/mapping/clins/camera_models/CataCamera.h>
...
void CornerTracker::track_features(cv::Mat img_1, cv::Mat img_2, Point2fVector& points1, Point2fVector& points2, IdVector& id1, IdVector& id2, const cv::Mat& mask){
  ...
  for(int i=0; i<status.size(); i++){
    ...
    if((pt.x<0)||(pt.y<0)||(pt.x>w-1)||(pt.y>h-1)){
      ...
      if(!mask.empty() && mask.at<uchar>(pt.y, pt.x) != 255){
        status.at(i) = 0;
      }
    }
    
    if(dist>25.0 || (status.at(i) == 0))	{
...
TrackHandler::TrackHandler(const std::string camera_intrincs_file)  {
  ransac_threshold_=0.0000002;
  next_feature_id_ = 0;
  gyro_accum_ = Eigen::Vector3f::Zero(); 
  n_gyro_readings_ = 0;

  camera_ = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(
      camera_intrincs_file);

  //TODO
  K_=(cv::Mat_<float> (3,3) << 669.8940458885896,0, 377.9459252967363, 
    0, 669.1450614220616, 279.63655686698144, 0, 0, 1);
  K_inv_ = K_.inv();
  ...
  tracker_.configure(21, 0.00001, 3, 30, 1.);
}
...
// void TrackHandler::reject_with_fundalmental(
//     const OutFeatureVector& un_prev_pts,
//     const OutFeatureVector& un_cur_pts, std::vector<uchar>& status){
//   // un_prev_pts and un_cur_pts are already on the normalized plane.
...
void TrackHandler::predict_features(){
  ...
  if (use_gyro_){
    ...
    /* Eigen::Matrix4d T_prev_in_cur = Eigen::Matrix4d::Identity();
    T_prev_in_cur.block(0,0,3,3) = T_cur_in_prev.block(0,0,3,3).transpose();
    T_prev_in_cur.block(0,3,3,1) = T_cur_in_prev.block(0,0,3,3).transpose()
        * (-T_cur_in_prev.block(0,3,3,1));
   
    Eigen::Vector3d pt_buf1, pt_buf2;

    for(auto& pt : prev_features_){
      pt_buf1<<pt.x, pt.y, 1.0;

      pt_buf2 = T_prev_in_cur.block(0,0,3,3) * pt_buf1 
                + T_prev_in_cur.block(0,3,3,1);

      const float x = pt_buf2[0] / pt_buf2[2];
      const float y = pt_buf2[1] / pt_buf2[2];
      cur_features_.emplace_back(x,y);
    } */
  }else{
...
void TrackHandler::tracked_features(
    OutFeatureVector& features, IdVector& feature_ids){
  // sanatize inputs
  ...
  if(prev_features_.size() != 0){
    ...
    tracker_.track_features(prev_img_, cur_img_,
                            prev_features_, cur_features_,
                            prev_feature_ids_, cur_feature_ids_, mask_);
  }
  ...
  if(cur_features_.size()>0){
    Point2fVector un_prev_pts, un_cur_pts;
    undistortPoints(prev_features_, un_prev_pts);
    undistortPoints(cur_features_, un_cur_pts);
   
    // reject with fundamental matrix.
    if (un_prev_pts.size() >= 15){
      assert(un_prev_pts.size() == un_cur_pts.size());
      assert(prev_feature_ids_.size() == cur_feature_ids_.size());
      std::vector<uchar> status;
      
      // tmp variables
      Point2fVector un_prev_pts_valid, un_cur_pts_valid;
      Point2fVector cur_features_valid, prev_features_valid;
      IdVector cur_id_valid, prev_id_valid;

      cv::findFundamentalMat(
        un_cur_pts, un_prev_pts, cv::FM_RANSAC, 1.0, 0.99, status);
      //std::cout<<"AAAA "<<status.size()<<","<<un_prev_pts.size()<<"\n";
      for(int k = 0; k < status.size(); ++k){
        if(status[k]){
          un_prev_pts_valid.emplace_back(un_prev_pts[k]);
          un_cur_pts_valid.emplace_back(un_cur_pts[k]);

          prev_id_valid.push_back(prev_feature_ids_[k]);
          cur_id_valid.push_back(cur_feature_ids_[k]);
          prev_features_valid.emplace_back(prev_features_[k]);
          cur_features_valid.emplace_back(cur_features_[k]);
        }
      }
      // copy back
      cur_features_ = cur_features_valid;
      prev_features_ = prev_features_valid;
      cur_feature_ids_ = cur_id_valid;
      prev_feature_ids_ = prev_id_valid;
      un_prev_pts = un_prev_pts_valid;
      un_cur_pts = un_cur_pts_valid;
      //std::cout<<"BBBBB "<<status.size()<<","<<un_prev_pts.size()<<"\n";
    }
    
    OutFeatureVector cur_features_norm;
    cur_features_norm.reserve(un_cur_pts.size());
    std::transform(un_cur_pts.begin(), un_cur_pts.end(), 
        std::back_inserter(cur_features_norm), [](const cv::Point2f& pt){
          return msckf_mono::Vector2<float>{ pt.x, pt.y };});
    ...
    // return features on the normalized plane.
    features.clear();
    std::copy(cur_features_norm.begin(), cur_features_norm.end(),
...
void TrackHandler::undistortPoints(Point2fVector& in, Point2fVector& out){
  ...
  for(const auto& pt: in){
    Eigen::Vector2d ept(pt.x, pt.y);
    Eigen::Vector3d un_ept;

    camera_->liftProjective(ept, un_ept);
    out.push_back(cv::Point2f(un_ept[0] / un_ept[2], un_ept[1] / un_ept[2]));
  }
}
