...
#include "cartographer/mapping/msckf/types.h"
#include "cartographer/mapping/msckf/matrix_utils.h"
...
private: 
  ...
  laserState<_S> laser_state_; // add laser
...
public:
  ...
  void propagate(imuReading<_S> &measurement_) {
    ...
    //协方差更新，与Jone Sola文档中给出的协方差更新方式有区别!
  ...
  // 增加新的相机位姿到状态变量，并增广相应的协方差矩阵；
  // 相机位姿由IMU的状态推算而得，因此其协方差传播由相应的公式可得（式14）；
  // 当需要移除某个相机位姿时，只需要删除对应的协方差矩阵块和对应的状态量即可。
  void augmentState(const int& state_id, const _S& time) {
    ...
    Quaternion<_S> q_LG = q_LI_ * imu_state_.q_IG;
    ...
    laser_state_.q_LG = q_LG;
    laser_state_.p_L_G =
      imu_state_.p_I_G + imu_state_.q_IG.inverse() * laser_state_.p_L_I;
    laser_state_.time = time;
    
    cam_state.time = time;
    cam_state.state_id = state_id;

    // Build MSCKF covariance matrix
    // 每次进行更新都需要重新开辟内存，将旧的数据填入
    if (cam_states_.size()) {
      int imu_state_size = 15;
      int cam_state_size = cam_covar_.cols();
      int laser_state_size = laser_covar_.cols();//6
      P_.resize(15 + cam_covar_.cols() + laser_state_size, 15 + cam_covar_.cols() + laser_state_size);
      ...
      /* P_.block(0, 15+cam_covar_.cols(), 15, laser_state_size) = imu_laser_covar_;
      P_.block(15+cam_covar_.cols(), 0, laser_state_size, 15) = imu_laser_covar_.transpose();
      P_.block(15+cam_covar_.rows(), 15+cam_covar_.cols(), imu_laser_covar_.rows(), imu_laser_covar_.cols()) = imu_laser_covar_; */
    } else {
      P_ = imu_covar_;//雷达与相机同步进来，没有相机数据也没有雷达数据
    }
    ...
    MatrixX<_S> J_L = MatrixX<_S>::Zero(6, 15 + 6 * cam_states_.size());
    J_L.template block<3, 3>(0, 0) = q_LI_.toRotationMatrix();
    J_L.template block<3, 3>(3, 0) =
      vectorToSkewSymmetric(imu_state_.q_IG.inverse() * p_LI_);
    J_L.template block<3, 3>(3, 12) = Matrix3<_S>::Identity();

    // Camera<_S> State Jacobian
    // MatrixX<_S> J = calcJ(imu_state_, cam_states_);

    MatrixX<_S> tempMat = MatrixX<_S>::Identity(15 + 6 * cam_states_.size() + 6 + 6/*laser*/,
                                                15 + 6 * cam_states_.size());
    tempMat.block(15 + 6 * cam_states_.size(), 0, 6,
                  15 + 6 * cam_states_.size()) = J;
    tempMat.block(15 + 6 * cam_states_.size() + 6, 0, 6,
                  15 + 6 * cam_states_.size()) = J_L;
    ...
    //在此处仿照相机增加IMU-LiDAR的协方差和位姿，但要注意，雷达仅与当前帧关联，这要如何操作呢？
    // Break everything into appropriate structs
    cam_states_.push_back(cam_state);
    imu_covar_ = P_aug.template block<15, 15>(0, 0);

    cam_covar_.resize(P_aug.rows() - 15 - 6, P_aug.cols() - 15 - 6);
    cam_covar_ = P_aug.block(15, 15, P_aug.rows() - 15 - 6, P_aug.cols() - 15 - 6);

    imu_cam_covar_.resize(15, P_aug.cols() - 15 - 6);
    imu_cam_covar_ = P_aug.block(0, 15, 15, P_aug.cols() - 15 - 6);
    
    laser_covar_.resize(6, 6);
    laser_covar_ = P_aug.block(15+cam_covar_.rows(), 15+cam_covar_.cols(), 6, 6);

    imu_laser_covar_.resize(15, 6);
    imu_laser_covar_ = P_aug.block(0, 15+cam_covar_.cols(), 15, 6);
  ...
  // 查找待边缘化的点，将它们从跟踪器中移除，并利用它们来更新观测到它们的相机位姿，执行EKF的观测方程更新
  void marginalize() {
    ...
    /***********************************************
     * 在此处增加雷达的观测残差模型,可否直接将之追加到H后面?
     * 直接追加不对，需要考虑不同的观测噪声，如何处理？
     * 仿照相机观测的协方差，将雷达的协方差追加到后面。
     * *********************************************/
    // laserMeasure();
...
private:
  ...
  // 对应原文“EKF Updates”部分，计算残差矩阵，计算卡尔曼增益K，根据K计算状态更新量，更新状态，更新协方差矩阵
