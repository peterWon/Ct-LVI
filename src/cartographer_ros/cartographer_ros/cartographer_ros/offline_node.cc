...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory) {
  ...
  while (playable_bag_multiplexer.IsMessageAvailable()) {
    ...
    // LOG(INFO)<<bag_topic.second << (it == bag_topic_to_sensor_id.end());
    ...
    if (it != bag_topic_to_sensor_id.end()) {
      ...
      if (msg.isType<sensor_msgs::Image>() 
        && bag_trajectory_options[0].use_image) {
        node.HandleImageMessage(
          trajectory_id, sensor_id, msg.instantiate<sensor_msgs::Image>());
      }
      if (msg.isType<sensor_msgs::CompressedImage>() 
        && bag_trajectory_options[0].use_compressed_image) {
        node.HandleImageMessage(
          trajectory_id, sensor_id, 
          msg.instantiate<sensor_msgs::CompressedImage>());
      }
      if (msg.isType<nav_msgs::Odometry>() && bag_trajectory_options[0].use_odometry) {
        node.HandleOdometryMessage(trajectory_id, sensor_id,
                                   msg.instantiate<nav_msgs::Odometry>());
      }
      if (msg.isType<sensor_msgs::NavSatFix>() && bag_trajectory_options[0].use_nav_sat) {
        node.HandleNavSatFixMessage(trajectory_id, sensor_id,
                                    msg.instantiate<sensor_msgs::NavSatFix>());
      }
      if (msg.isType<cartographer_ros_msgs::LandmarkList>() &&   bag_trajectory_options[0].use_landmarks) {
