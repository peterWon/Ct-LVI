...
Node::Node(
  ...
  if(node_options_.enable_cache_range_data){
...
void Node::AddSensorSamplers(const int trajectory_id,
  ...
  sensor_samplers_.emplace(
      ...
      std::forward_as_tuple(
          ...
          options.landmarks_sampling_ratio, options.image_sampling_ratio));
}
...
Node::ComputeExpectedSensorIds(
  ...
  if (options.use_image) {
    expected_topics.insert(SensorId{SensorType::IMAGE, kImageTopic});
  }else if (options.use_compressed_image) {
    expected_topics.insert(SensorId{SensorType::IMAGE, kImageTopic});
  }
...
void Node::LaunchSubscribers(const TrajectoryOptions& options,
  ...
  if (options.use_image) {
    CHECK(!options.use_compressed_image)<<"Do not enable use_image and use_compressed_image at the same time.";
    std::string topic = topics.image_topic;
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::Image>(
             &Node::HandleImageMessage, trajectory_id, topic, &node_handle_,
             this),
         topic});
  }
  if (options.use_compressed_image){
    CHECK(!options.use_image)<<"Do not enable use_image and use_compressed_image at the same time.";
    
    std::string topic = topics.image_topic;
    LOG(INFO)<<"connected "<<topic;
    subscribers_[trajectory_id].push_back(
        {SubscribeWithHandler<sensor_msgs::CompressedImage>(
             &Node::HandleImageMessage, trajectory_id, topic, &node_handle_,
             this),
         topic});
  }
...
void Node::HandleImageMessage(
  ...
  carto::common::MutexLocker lock(&mutex_);
  if (!sensor_samplers_.at(trajectory_id).image_sampler.Pulse()) {
    return;
  }
  map_builder_bridge_.sensor_bridge(trajectory_id)
      ->HandleImageMessage(sensor_id, msg);
}
