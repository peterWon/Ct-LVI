...
//       const std::string& sensor_id, const sensor_msgs::Image::ConstPtr& msg);
  template<typename RosImagePtr>
  void HandleImageMessage(const std::string& sensor_id, const RosImagePtr& msg){
    cartographer::sensor::ImageData img_data = ToImageData(msg);
    if(!img_data.img.empty()){
      // LOG(INFO)<<img_data.img.rows<<","<<img_data.img.cols;
      trajectory_builder_->AddSensorData(sensor_id, img_data);
    }else{
      LOG(WARNING)<<"Convert image failed!";
    }
  }
