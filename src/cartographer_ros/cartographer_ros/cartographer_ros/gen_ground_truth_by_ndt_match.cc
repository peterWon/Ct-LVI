...
void RunForFastRotationBag(
  ...
  // 取bag后10%的数据与前10%进行配准，将配准成功的写入结果文件作为Ground Truth参考
  size_t cnt = stamps_in_bag.size();
  for(size_t i = cnt - 1; i > cnt * 0.9; --i){
    for(size_t j = 0; j < cnt * 0.1; --j){
      pcl::PointCloud<pcl::PointXYZ>::Ptr pc_from = clouds[i];
      pcl::PointCloud<pcl::PointXYZ>::Ptr pc_to = clouds[j];
      ...
      float align_error = icp.getFitnessScore();

      if(!FLAGS_viewer.empty() && align_error < 1.0){
        pcl::transformPointCloud(*pc_from, *output_cloud, trans);
        boost::shared_ptr<pcl::visualization::PCLVisualizer>
        viewer_final(new pcl::visualization::PCLVisualizer("3D Viewer"));
        viewer_final->setBackgroundColor(0, 0, 0);
        //对目标点云着色（红色）并可视化
        const auto target_cloud = pc_to;
      ...
      ofs << stamps_in_bag[i] << " " << stamps_in_bag[j] << " "<< align_error <<" "
...
int main(int argc, char** argv) {
  ...
  ::cartographer_ros::RunForFastRotationBag(
    FLAGS_bag_filename, FLAGS_ground_truth_filename);
}
