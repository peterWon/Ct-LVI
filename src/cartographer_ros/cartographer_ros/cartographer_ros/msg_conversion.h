...
// cartographer::sensor::ImageData ToImageData(
//     const sensor_msgs::Image::ConstPtr& msg);

// both for sensor_msgs::Image and sensor_msgs::CompressedImage
template <typename RosImagePtr>
cartographer::sensor::ImageData ToImageData(const RosImagePtr& msg){
  const cartographer::common::Time time = FromRos(msg->header.stamp);
  cartographer::sensor::ImageData ret;
  ret.time = time;
  ret.img = cv::Mat();
  ...
  if(cv_ptr->image.channels()==3){
    cv::Mat temp_img;
    cv::cvtColor(cv_ptr->image, temp_img, CV_BGR2GRAY);
    temp_img.copyTo(ret.img);
  }else{
    cv_ptr->image.copyTo(ret.img);
  }
  return ret;
}
