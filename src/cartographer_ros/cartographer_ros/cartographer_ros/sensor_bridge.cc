...
void SensorBridge::HandlePointCloud2Message(
  ...
  if(sensor_type == "ouster"){
    ...
    // LOG(INFO)<<rel_time_last<<","<<point_cloud.front()[3]<<","<<point_cloud.back()[3];
...
void SensorBridge::HandleRangefinder(
  ...
  // LOG(INFO)<<frame_id<<" pos: "<<sensor_to_tracking->translation().transpose();
  // LOG(INFO)<<frame_id<<" rot: "<<sensor_to_tracking->rotation().toRotationMatrix();
